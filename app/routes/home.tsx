...
export default function Portfolio() {
  ...
  // 🟢 UPDATED: Submission Handler targeting Vercel API Route
  ...
  // Intersection Observer (Scroll Fix)
  useEffect(() => {
    if (typeof window === "undefined") return;
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100">
      ...
      <main className="relative mx-auto mt-6 flex max-w-6xl flex-col gap-14 px-4 pb-16 sm:px-6 lg:px-8 lg:pb-20">
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 pt-4">
              {[
                ...
                <span
                  key={tag}
                  className="rounded-full border border-slate-200/60 bg-white/60 px-4 py-1 text-xs text-slate-700 dark:border-slate-700/60 dark:bg-slate-800/60 dark:text-slate-300"
                >
                  {tag}
                </span>
              ))}
        ...
        <section id="projects" className="space-y-4">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {projects.map((project, index) => (
              ...
              >
                {/* Project image container with centered logo */}
        ...
        {/* Footer */}
        <footer className="mt-2 flex flex-col items-center justify-between gap-2 border-t border-white/60 pt-4 text-[12px] text-slate-500 sm:flex-row dark:border-slate-700/60 dark:text-slate-400">
